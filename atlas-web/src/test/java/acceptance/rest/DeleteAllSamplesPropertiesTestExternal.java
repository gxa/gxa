...
public class DeleteAllSamplesPropertiesTestExternal extends CuratorApiTestExternal {
    ...
    public void shouldRemoveOnlyTheSpecifiedProperties() throws Exception {
        ...
        deleteAllSamplesProperties();

        String modifiedExperiment = get(MODIFIED_EXPERIMENT_URI).asString();

        with(modifiedExperiment)
            .assertThat("$..samples.properties[?(@.name=='sex')]", is(empty()))
            .and()
    ...
    @Test
    public void shouldNotAddPropertiesToAssays() throws Exception {
    ...
    private void deleteAllSamplesProperties() throws Exception {
        given().header("Content-Type", "application/json")
            .body(JSON_PROPERTIES)
