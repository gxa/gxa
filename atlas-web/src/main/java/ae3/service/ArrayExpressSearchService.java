...
public class ArrayExpressSearchService implements InitializingBean {
    ...
    public void setAtlasDataSource(DataSource atlasDataSource) {
        this.atlasDataSource = atlasDataSource;
    }
    ...
    public void initialize() {
        try {
            ...
            new Thread() {
                public void run() {
                    squeryService.getEfvListHelper().preloadData();
                }
            }.start();
            new Thread() {
                public void run() {
                    squeryService.getEfoListHelper().preloadData();
                }
            }.start();
            new Thread() {
                public void run() {
                    squeryService.getGeneListHelper().preloadData();
                }
            }.start();

            atlasQueryRunner = new QueryRunner(atlasDataSource);
        }
        catch (Exception e) {
