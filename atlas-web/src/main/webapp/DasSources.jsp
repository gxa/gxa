...
<SOURCES>
    ...
    <VERSION uri="gxa" created="2009-07-13">
      <COORDINATES uri="http://www.dasregistry.org/coordsys/CS_DS95" source="Gene_ID" authority="Ensembl" test_range="ID">Gene_ID</COORDINATES>
      <CAPABILITY type="das1:features" query_uri="<%= URLUtil.getDasS4Url(request) %>/features" />
      <CAPABILITY type="das1:types" query_uri="<%= URLUtil.getDasS4Url(request) %>/types" />
      <CAPABILITY type="das1:unknown-segment" query_uri="<%= URLUtil.getDasS4Url(request) %>/features" />
     </VERSION>
   </SOURCE>
