 ...
 * <p/>
 * Tag: Sample Name <br/>Allowed attributes: Characteristics, Material Type,
 * Description, Comment, Array Design File/Ref
 *
 * @author Tony Burdett
 * @date 22-Jan-2009
 */
public class SampleHandler extends AbstractSDRFHandler
  ...
  public void readValues() throws ParseException {
    ...
    if (headers[0].equals(tag)) {
      ...
      synchronized (investigation.SDRF) {
        sample = investigation.SDRF.lookupNode(values[0], SampleNode.class);
        if (sample == null) {
          sample = new SampleNode();
          sample.setNodeType(headers[0]);
          sample.setNodeName(values[0]);
          investigation.SDRF.storeNode(sample);
          addNextNodeForCompilation(sample);
        }
      }
      ...
      for (int i = 1; i < values.length;) {
        if (headers[i].startsWith("characteristics")) {
          CharacteristicsHandler handler = new CharacteristicsHandler();
          i += handleAttribute(sample, handler, headers, values, i);
        }
        else if (headers[i].equals("materialtype")) {
          MaterialTypeHandler handler = new MaterialTypeHandler();
          i += handleAttribute(sample, handler, headers, values, i);
      ...
      // iterated over every column, so must have reached the end
      // update node in SDRF
      investigation.SDRF.updateNode(sample);
