 ...
 * <p/>
 * Tag: Derived Array Data File Allowed attributes: Array Design File/Ref,
 * Comment
 *
 * @author Tony Burdett
 * @date 22-Jan-2009
 */
public class DerivedArrayDataHandler extends AbstractSDRFHandler
    implements SDRFDatatypeHandler {
  public DerivedArrayDataHandler() {
    setTag("derivedarraydatafile");
  }
  ...
  public void readValues() throws ParseException {
    ...
    if (headers[0].equals(tag)) {
      ...
      synchronized (investigation.SDRF) {
        dadf = investigation.SDRF.lookupNode(values[0],
                                             DerivedArrayDataNode.class);
        if (dadf == null) {
          dadf = new DerivedArrayDataNode();
          dadf.setNodeType(headers[0]);
          dadf.setNodeName(values[0]);
          investigation.SDRF.storeNode(dadf);
          addNextNodeForCompilation(dadf);
        }
      }
      ...
      for (int i = 1; i < values.length;) {
        if (headers[i].startsWith("comment")) {
          ...
          dadf.comments.put(type, values[i]);
        }
        else if (headers[i].startsWith("arraydesign")) {
          ArrayDesignHandler handler;
          if (headers[i].endsWith("file")) {
          ...
          i += handleAttribute(dadf, handler, headers, values, i);
        }
        else if (headers[i].equals("")) {
          // skip the case where the header is an empty string
      ...
      // iterated over every column, so must have reached the end
      // update node in SDRF
      investigation.SDRF.updateNode(dadf);
