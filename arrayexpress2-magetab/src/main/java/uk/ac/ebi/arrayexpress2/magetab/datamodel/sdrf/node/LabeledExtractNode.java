...
/**
 * A labeled extract node in the graph. Labeled Extract nodes are top level
 * nodes and can have associated characteristics, material types, descriptions,
 * labels, and sets of comments.
 *
 * @author Tony Burdett
 * @date 22-Jan-2009
 */
public class LabeledExtractNode extends AbstractSDRFNode {
  ...
  public String[] headers() {
    ...
    headersList.add("Labeled Extract Name");
    if (label != null) {
      Collections.addAll(headersList, label.headers());
    }
  ...
  public String[] values() {
    ...
    if (label != null) {
      Collections.addAll(valuesList, label.values());
    }
    for (CharacteristicsAttribute c : characteristics) {
