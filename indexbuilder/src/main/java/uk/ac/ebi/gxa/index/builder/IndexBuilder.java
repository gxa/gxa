...
public interface IndexBuilder<T> {
    ...
    /**
     * Set the location for the index.  If there is already a pre-existing index at this location, implementations should
     * update this index.  If there is no index pre-existing, it should be created.
     *
    ...
    /**
     * Flags that only experiments or genes pending indexing should be included. The pending flag is set within the Atlas
     * database as a means of tracking all experiments that have been loaded but not yet indexed. Setting "pending mode"
     * to true indicates you only wish to index these experiments.  Note that you would normally only ever use pending
     * mode whne updating the index.
     * <p/>
     * This is false by default.
     *
     * @param pendingMode include only those experiments or genes flagged in the backing database as "pending".  True
     *                    indicates only pending experiments are used, false indicates all.
     */
    void setPendingMode(boolean pendingMode);

    /**
     * Indicates whether experiments are included in the construction of this index.  The pending flag indicates whether
     * you wish to include only experiments marked as pending, or all of them.
     *
     * @return include only those experiments flagged in the backing database as "pending".  True indicates only pending
    ...
    /**
     * Build the index.  This will build the index entirely from scratch.  Use this if you wish to create or recreate the
     * index with up-to-date information from the backing database.  If you wish to update the index with only
     * pre-existing genes and experiments, use {@link #updateIndex()}.
     * <p/>
     * Note that this method is not guaranteed to be synchronous, it only guarantees that the index has started building.
     * Implementations are free to define their own multithreaded strategies for index construction.  If you wish to be
     * notified on completion, you should register a listener to get callback events when the build completes by using
     * {@link #buildIndex(uk.ac.ebi.gxa.index.builder.listener.IndexBuilderListener)}. You can also use a listener to get
     * at any errors that may have occurred during index building.
     * <p/>
    ...
    /**
     * Build the index and register a listener that provides a callback on completion of the build task.  This will build
     * the index entirely from scratch.  Use this if you wish to create or recreate the index with up-to-date information
     * from the backing database.  If you wish to update the index with only pre-existing genes and experiments, use
     * {@link #updateIndex()}.
     * <p/>
     * Note that this method is not guaranteed to be synchronous, it only guarantees that the index has started building.
     * Implementations are free to define their own multithreaded strategies for index construction.
     *
    ...
    /**
     * Incrementally builds the index, updating the existing items rather than building from scratch.
     * <p/>
     * Note that this method is not guaranteed to be synchronous, it only guarantees that the index has started updating.
     * Implementations are free to define their own multithreaded strategies for index construction. If you wish to be
     * notified on completion, you should register a listener to get callback events when the update completes by using
     * {@link #updateIndex(uk.ac.ebi.gxa.index.builder.listener.IndexBuilderListener)}. You can also use a listener to get
     * at any errors that may have occurred during index building.
     * <p/>
