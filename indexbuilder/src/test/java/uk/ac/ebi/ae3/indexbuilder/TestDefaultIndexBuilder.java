...
/**
 * A test case fo assessing whether the DefaultIndexBuilder class initializes
 * correctly and can run a build of the index over some dummy data from the test
 * database.  Note that only very simple checks are run to ensure that some data
 * has gone into the index.  Precise implementations tests should be done on the
 * individual index building services, not this class.  This test is just to
 * ensure clean startup and shutdown of the main index builder.
 *
