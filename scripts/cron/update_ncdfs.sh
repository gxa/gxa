...
echo `eval date +%HH%MM%ss`": The following experiments will have their ncdfs updated: "  >> $process_file.log
# update ncdfs
curl -X POST -b $authentication_cookie -H "Accept: application/json" "http://$3:$4/$5/admin?op=schedulesearchexp&runMode=RESTART&type=updateexperiment&pendingOnly=INCOMPLETE_NETCDF&autoDepends=false&indent" 2>&1 >> $process_file.log
