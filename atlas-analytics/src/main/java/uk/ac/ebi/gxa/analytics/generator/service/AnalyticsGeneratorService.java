 ...
 * FIXME
 *
 * An abstract AnalyticsGeneratorService, that provides convenience methods for
 * getting and setting parameters required across all AnalyticsGenerator
 * implementations.  This class is typed by the type of the repository backing
 * this AnalyticsGeneratorService - this may be a file, a datasource, an FTP
 ...
 * #createAnalytics()} ()} ()} which contains the logic for constructing the
 * relevant parts of the index for each implementation.  Clients should call
 * {@link #generateAnalytics()} ()} to trigger Analytics construction.  At the moment,
...
public abstract class AnalyticsGeneratorService<T> {
  ...
  public AnalyticsGeneratorService(AtlasDAO atlasDAO, T repositoryLocation) {
  ...
  protected abstract void createAnalyticsForExperiment(
      String experimentAccession) throws AnalyticsGeneratorException;
    ...
    try {
      ...
      InputStream in = getClass().getClassLoader().
          getResourceAsStream("META-INF/maven/uk.ac.ebi.microarray.atlas/" +
              "atlas-analytics/pom.properties");
      properties.load(in);
