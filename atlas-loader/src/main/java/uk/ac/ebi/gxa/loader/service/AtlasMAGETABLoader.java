...
public class AtlasMAGETABLoader {
    ...
    public static boolean isHTS(MAGETABInvestigation investigation) {
        ...
        if (scanNodes.size() == 0) {
            log.info("No comment scan nodes found - investigation: " + investigation.accession + " not HTS");
            return false;
        }
        for (ScanNode scanNode : scanNodes) {
            if (!(scanNode.comments.keySet().contains("ENA_RUN") && scanNode.comments.containsKey("FASTQ_URI"))) {
                log.info("No comment[ENA_RUN] found - investigation: " + investigation.accession + " not HTS");
                return false;
            }
        }
        return true;
