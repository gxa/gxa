...
public class NetCDF2MAGETAB {
    public static MAGETABInvestigation convert(NetCDFProxy proxy) throws Exception{
          MAGETABInvestigation investigation  = new MAGETABInvestigation();

          investigation.accession = proxy.getExperiment();

          AssayNode node = new AssayNode();
          node.setNodeName("assayNode");
          node.setNodeType("Assay");

          //SampleNode node = new

          //node.addChildNode();
        
          //ArrayDesignNode ad_node = new ArrayDesignNode();
          //node.setNodeName("adNode");
          //node.setNodeType("ArrayDesign");
        
          //node.arrayDesigns.add(ad_node);

          investigation.SDRF.storeNode(node);
    ...
    public static void loadFileToCache(java.net.URL url, AtlasLoadCache cache) throws Exception{
        ...
        for(int iDesignElement = 0; iDesignElement!=designElementAccessions.length; iDesignElement++ ){
           ...
           storage.add(designElementAccessions[iDesignElement],new Iterator<Float>(){
               ...
               public Float next(){
                   return new Float(expressionData[iCurrent++]);
               }
       ...
       for(String designElement : proxy.getDesignElementAccessions()){
           ...
           pvalStorage.add(designElement,new Iterator<Float>(){ //factorize me!
               ...
               public Float next(){
                   return new Float(pvalData[iCurrent++]);
               }
               public boolean hasNext(){
           ...
           tstatStorage.add(designElement,new Iterator<Float>(){ //factorize me!
               ...
               public Float next(){
                   return new Float(tstatData[iCurrent++]);
               }
               public boolean hasNext(){
       ...
       for(int iUniquePV = 0 ; iUniquePV !=numOfPVs; iUniquePV++){
           ...
           pvalMap.put(new Pair<String, String>(uP, uPV),
                                new DataMatrixStorage.ColumnRef(pvalStorage, iUniquePV));

           tstatMap.put(new Pair<String, String>(uP, uPV),
                                new DataMatrixStorage.ColumnRef(tstatStorage, iUniquePV));
       }
