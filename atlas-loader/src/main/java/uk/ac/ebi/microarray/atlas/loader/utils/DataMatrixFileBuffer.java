...
/**
 * A class that can be used to buffer data read from a MAGE-TAB Derived Array
 * Data Matrix format file.  This is accomplished by use of a factory method to
 * generate a single DataMatrixFileBuffer object per-file.  On construction, the
...
public class DataMatrixFileBuffer {
  ...
  /**
   * Read off expression values for the given assay ref.  This buffer object
   * automatically knows which columns must be read, as a dictionary lookup on
   * column names was performed on initialization.  You can configure the
   * dictionary of terms to use manually - for more on this see {@link
   * QuantitationTypeDictionary}.  This method blocks until initialization has
   * completed, and this buffer knows which columns to read to obtain expression
   * values.
   *
...
//        new HashMap<String, List<ExpressionValue>>();
      ...
      new Thread(new Runnable() {
        public void run() {
          ...
          try {
            ...
            QuantitationTypeDictionary dictionary =
                QuantitationTypeDictionary.getQTDictionary();
            for (Header header : headers) {
              ...
              for (String qtType : header.getQuantitationTypes()) {
                ...
                if (dictionary.lookupTerm(qtType)) {
                  ...
                  if (!assayRefToEVColumn.containsKey(header.assayRef)) {
                    ...
                    assayRefToEVColumn.put(header.assayRef,
              ...
              if (possibleTypes.size() > 1) {
                ...
                String message =
                    "Unable to load - data matrix file contains multiple " +
                        "candidate quantitation types to use for expression " +
                        "values, could be one of: " + sb.toString();
                ErrorItem error =
                    ErrorItemFactory
                        .getErrorItemFactory(getClass().getClassLoader())
                        .generateErrorItem(
  ...
  private Header[] parseHeaders(BufferedReader reader)
      ...
      if (!line.trim().equals("")) {
        if (!line.startsWith("#")) {
          ...
          while (MAGETABUtils.endsWithEscapedNewline(firstLine)) {
            ...
            line = MAGETABUtils.removeEscapedNewlines(firstLine, secondLine);
    ...
    if (!title.startsWith("hybridizationref")) {
      // this file looks wrong, so generate error item and throw exception
      String message =
          "Failed to parse the derived array data matrix file - the " +
              "first line started with '" + title + "' when " +
              "'hybridizationref' was expected";
      ErrorItem error =
          ErrorItemFactory
              .getErrorItemFactory(getClass().getClassLoader())
              .generateErrorItem(
    ...
    Header header = null;
    for (int column = 1; column < hybRefs.length; column++) {
      // grab the normalized values
      String hybRef = hybRefs[column];
