...
public class AtlasLoadCache {
  ...
  /**
   * Creates a new cache for storing objects that are to be loaded into the
   * database.
   */
  ...
  /**
   * Adds an experiment to the cache of objects to be loaded.  Experiments are
   * indexed by accession, so every experiment in the cache should have a unique
   * accession.  If an experiment is passed to this method with an accession
   * that is the same as one that has been previously stored, and the experiment
   * is not the same object, then an IllegalArgumentException is thrown
   *
  ...
  /**
   * Retrieves an experiment from the load cache with the given accession, if
   * present.  If there is no experiment with the given accession, null is
   * returned.
   *
   * @param accession the accession of the experiment to fetch
   * @return the experiment, if present, or null if there is no experiment with
   *         this accession
   */
  ...
  /**
   * Adds an assay to the cache of objects to be loaded.  Assays are indexed by
   * accession, so every assay in the cache should have a unique accession.  If
   * an assay is passed to this method with an accession that is the same as one
   * that has been previously stored, and the assay is not the same object, then
  ...
  /**
   * Retrieves an assay from the load cache with the given accession, if
   * present.  If there is no assay with the given accession, null is returned.
   *
   * @param accession the accession of the assay to fetch
   * @return the assay, if present, or null if there is no assay with this
   *         accession
   */
  ...
  /**
   * Adds an sample to the cache of objects to be loaded.  Samples are indexed
   * by accession, so every sample in the cache should have a unique accession.
   * If an sample is passed to this method with an accession that is the same as
   * one that has been previously stored, and the sample is not the same object,
  ...
  /**
   * Retrieves an sample from the load cache with the given accession, if
   * present.  If there is no sample with the given accession, null is
   * returned.
   *
   * @param accession the accession of the sample to fetch
   * @return the sample, if present, or null if there is no sample with this
   *         accession
   */
  ...
  /**
   * Clears the cache.  All objects currently stored in this cache will be
   * removed.
   */
