...
/**
 * A {@link uk.ac.ebi.microarray.atlas.netcdf.helper.CallableSlicer}
...
public class ExperimentSlicer extends CallableSlicer<Set<DataSlice>> {
  ...
  public Set<DataSlice> call() throws Exception {
    ...
    for (final ArrayDesign arrayDesign : arrays) {
      ...
      arraySlicer.setAtlasDAO(
          getAtlasDAO());
      arraySlicer.setGeneFetchingStrategy(
          fetchGenesTask, unmappedGenes);
      arraySlicer.setAnalyticsFetchingStrategy(
    ...
    synchronized (arrayFetching) {
      getLog().debug("Waiting for all array slicing tasks to complete " +
    ...
    // and return
    getLog().debug("Compiled the set of all dataslices for " +
