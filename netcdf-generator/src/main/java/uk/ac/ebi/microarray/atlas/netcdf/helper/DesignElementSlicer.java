...
public class DesignElementSlicer extends CallableSlicer<Void> {
  ...
  public DesignElementSlicer(ExecutorService service, ArrayDesign arrayDesign,
                              DataSlice dataSlice) {
    super(service);
  ...
  public Void call() throws Exception {
    ...
    getLog().debug("Indexing gene data by design element ID for " +
        arrayDesign.getAccession());
    fetchGenesTask.get();
    ...
    // compare to design elements and store, correctly indexed
    getLog().debug(
        "Indexing analytics data by design element ID for " +
            arrayDesign.getAccession());
    fetchAnalyticsTask.get();
    getLog().debug(
        "Analytics data for " + arrayDesign.getAccession() + " acquired");
    ...
    getLog().debug("Design Element/Gene/Analytics data for " +
