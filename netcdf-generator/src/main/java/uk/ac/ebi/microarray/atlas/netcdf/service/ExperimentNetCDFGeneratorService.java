...
public class ExperimentNetCDFGeneratorService
  ...
  protected void createNetCDFDocs() throws NetCDFGeneratorException {
    ...
    for (Experiment experiment : experiments) {
      if (getUpdateMode()) {
        // query for experiments to update
...
//        "select unique experiment_accession from AE1__EXPERIMENT__MAIN where experiment_accession is not null";
  ...
  public void prepareStatements() throws SQLException {
    ...
    stmtGetExpQuantType = connection
        .prepareStatement("select exp.experiment_id_key, meas.value " +
    ...
    stmtGetAryDes = connection.prepareStatement(
        "select UNIQUE m.ARRAYDESIGN_ID from AE1__ASSAY__MAIN m where m.ARRAYDESIGN_ID is not null AND EXPERIMENT_ACCESSION = ?");
    stmtGetGenes = connection.prepareStatement(
        "select gene_identifier, designelement_id_key, gene_id_key from AE2__DESIGNELEMENT__MAIN t where t.ARRAYDESIGN_ID = ? order by gene_id_key, designelement_id_key");
    stmtGetSamples = connection.prepareStatement(
        "select UNIQUE sample_id_key, arraydesign_accession, arraydesign_name, assay_id_key from AE1__SAMPLE__MAIN t where t.ARRAYDESIGN_ID = ? AND experiment_id_key = ? order by sample_id_key");
    stmtGetAssays = connection.prepareStatement(
        "select UNIQUE assay_id_key from AE1__ASSAY__MAIN t where t.ARRAYDESIGN_ID = ? AND experiment_id_key = ? order by assay_id_key");
    stmtGetAtlas = connection.prepareStatement(
        "select designelement_id_key, gene_id_key, updn_pvaladj, updn_tstat from atlas where experiment_id_key = ? AND arraydesign_id_key = ? AND ef = ? AND efv = ?");
    stmtGetDataAbs = connection.prepareStatement(
        "select v.absolute, v.designelement_id_key, v.sample_id, de.gene_id_key, v.assay_id from AE2__EXPRESSIONVALUE__MAIN v, ae2__designelement__main de where v.ARRAYDESIGN_ID = ? and v.experiment_id = ? and de.arraydesign_id=v.ARRAYDESIGN_ID and v.absolute is not null and v.designelement_id_key is not null and v.sample_id is not null and de.designelement_id_key=v.designelement_id_key");
    stmtGetDataRat = connection.prepareStatement(
        "select v.ratio, v.designelement_id_key, v.sample_id, de.gene_id_key, v.assay_id  from AE2__EXPRESSIONVALUE__MAIN v, ae2__designelement__main de where v.ARRAYDESIGN_ID = ? and v.experiment_id = ? and de.arraydesign_id=v.ARRAYDESIGN_ID and v.ratio is not null and v.designelement_id_key is not null and v.assay_id is not null and de.designelement_id_key=v.designelement_id_key");
   ...
   */
  public void createForExperiment(String accession) throws SQLException,
   ...
   */
  void createForArray(String array_id, String experiment_id, String accession,
                      String qtType)
      throws SQLException, IOException, InvalidRangeException {

    log.info(
        "[exp:" + accession + "] " + "..Array " + array_id + " Experiment " +
            experiment_id);

    if (skipExisting) {
      String fileName1 =
          netcdfPath + File.separator + experiment_id + "_" + array_id + ".nc";
      String fileName2 =
          netcdfPath + File.separator + experiment_id + "_" + array_id +
              "_ratios.nc";
    ...
    TreeMap des = new TreeMap(new Comparator() {
    ...
    while (rset.next()) {
      ...
      Integer[] dege = new Integer[]{new Integer(designelement_id_key),
                                     new Integer(gene_id_key)};
    ...
    rset.close();
    log.info(
        "[exp:" + accession + "] " + "......" + Integer.toString(gene_counter));
    ...
    rset.close();
    log.info("[exp:" + accession + "] " + "......" +
        Integer.toString(sample_counter));
    ...
    rset.close();
    log.info("[exp:" + accession + "] " + "......" +
        Integer.toString(assay_counter));
    ...
    while (fIter.hasNext()) {
      ...
      Vector fValuesForAssays =
          getEFValuesForAssays(fName, experiment_id, array_id);
      Vector fValuesForSamples =
          getEFValuesForSamples(fName, experiment_id, array_id);
      ...
      if (fValuesForSamples.size() > 0) {
        efvs4samples.put(fName, fValuesForSamples);
      }
    ...
    while (cIter.hasNext()) {
      ...
      LinkedHashMap<String, ArrayList<String>> charValuesForSamples =
          getCharValuesForSamples(characteristic, experiment_id, array_id);
    ...
    createMatrix(
        netcdfPath + File.separator + experiment_id + "_" + array_id + ".nc",
        sample_counter, assay_counter, gene_counter, samples, assays,
        des, des_vector, efvs4assays, characValues4samples, stmtGetDataAbs,
        accession, qtType, adAccession, adName, samples2assays, uniqEfvsNums,
        uniqEfvs, pvalues, tstats);

    /* ratios - map to BioAssays */

    stmtGetDataRat.setString(1, array_id);
    stmtGetDataRat.setString(2, experiment_id);
    createMatrix(netcdfPath + File.separator + experiment_id + "_" + array_id +
        "_ratios.nc", sample_counter, assay_counter, gene_counter, samples,
                 assays,
                 des, des_vector, efvs4assays, characValues4samples,
                 stmtGetDataRat,
                 accession, qtType, adAccession, adName, samples2assays,
                 uniqEfvsNums, uniqEfvs, pvalues, tstats);
  ...
  void getAssaySampleProperties(Connection connection) throws SQLException {
    ...
    try {
      br = new BufferedReader(new InputStreamReader(
          getClass().getClassLoader().getResourceAsStream("mappings.txt")));
    }
    catch (Exception e) {
    ...
    try {
      ...
      while (line != null && line.compareTo("") != 0) {
        ...
        if (attributeName.toLowerCase().contains("assay")) {
          transHash.put(filterName, connection.prepareStatement(
              "select unique ef.value, a.assay_id_key from " + attributeName +
                  " ef, ae1__assay__main a" +
          ...
          transHash2.put(filterName, connection.prepareStatement(
              "select unique ef.value, s.sample_id_key from " + attributeName +
                  " ef, ae1__sample__main s" +
                  " where ef.EXPERIMENT_ID_KEY=?" +
        ...
        if (attributeName.toLowerCase().contains("sample")) {
          transHash3.put(filterName, connection.prepareStatement(
              "select unique charac.value, s.sample_id_key " +
  ...
  /**
   * returns all experimental factor values for a given factor
   * name/experiment/bioassays combination
   */
  Vector getEFValuesForAssays(String fName, String experiment_id,
                              String array_id) throws SQLException {
  ...
  /**
   * returns all experimental factor values for a given factor
   * name/experiment/biosamples combination
   */
  Vector getEFValuesForSamples(String fName, String experiment_id,
                               String array_id) throws SQLException {
  ...
  LinkedHashMap<String, ArrayList<String>> getCharValuesForSamples(
      String characteristic, String experiment_id, String array_id)
      throws SQLException {
...
//    Vector charValues = new Vector();
    ...
    LinkedHashMap<String, ArrayList<String>> charValues =
   ...
   * @param designElementsVector - vector of design elements, preserving BDC row
   *                             order
   * @param stmt                 query to retrieve data for the experiment/assays
   *                             combination
   * @throws java.lang.Exception ...
   */
  void createMatrix(String fileName, int sampleCount, int assayCount,
                    int geneCount, TreeMap samples, TreeMap assays,
                    TreeMap designElements, Vector designElementsVector,
                    TreeMap expFactorValues, TreeMap sampleCharacValues,
                    PreparedStatement stmt, String accession, String qtType,
                    String adAccession, String adName,
                    TreeMap samples2assays,
                    TreeMap uniqEfvsNums, Vector uniqEfvs, Vector pvalues,
                    Vector tstats)
      throws SQLException, IOException, InvalidRangeException {
    ...
    if (sampleCharacCount > 0) {
      ...
      String[] sampleCharacteristics =
          (String[]) sampleCharacValues.keySet().toArray(new String[0]);
      String charVal = "";
      for (int i = 0; i < sampleCharacCount; i++) {
        if (sampleCharacteristics[i].length() > characLen) {
          characLen = sampleCharacteristics[i].length();
        }
        LinkedHashMap<String, ArrayList<String>> sCharVals =
            (LinkedHashMap<String, ArrayList<String>>) sampleCharacValues
                .get(sampleCharacteristics[i]);
        for (ArrayList<String> values : sCharVals.values()) {
          charVal = StringUtils.join(values.iterator(), ",");
          if (charVal.length() > characLen) {
            characLen = charVal.length();
          }
        }
      }
    ...
    int k = 0;
    while (iter.hasNext()) {
      iter.setDoubleNext((Double) pvalues.get(k++));
    }
    ...
    k = 0;
    while (iter.hasNext()) {
      iter.setDoubleNext((Double) tstats.get(k++));
    }
    ...
    log.info(
        "[exp:" + accession + "] " + "Expecting... " + total + " elements");
    do {
    ...
    if (sampleCharacCount > 0) {
      ...
      ArrayChar SCV =
          new ArrayChar.D3(sampleCharacCount, sampleCount, characLen);
      ...
      for (int i = 0; i < sampleCharacCount; i++) {
        ...
        LinkedHashMap<String, ArrayList<String>> sCharVals =
            (LinkedHashMap<String, ArrayList<String>>) sampleCharacValues
                .get(scs[i]);
        log.info(
            "[exp:" + accession + "] " + "Adding characteristic values for " +
                scs[i]);
