...
public class AssaySlicer extends CallableSlicer<Void> {
    ...
    public AssaySlicer(ExecutorService service, Experiment experiment, ArrayDesign arrayDesign, DataSlice dataSlice) {
        super(service);
    ...
    public Void call() throws Exception {
        ...
        List<Assay> assays =
                getAtlasDAO().getAssaysByExperimentAndArray(experiment.getAccession(), arrayDesign.getAccession());
        // and store
        dataSlice.storeAssays(assays);

        getLog().debug("Fetching samples data for each assay on " + arrayDesign.getAccession());
        ...
        getLog().debug("Assay and Sample data for " + arrayDesign.getAccession() + " stored");
        return null;
    }
}
